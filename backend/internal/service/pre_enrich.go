...
var typeKeywords = []typeKeywordEntry{
	...
	{"Благодарность", []string{
		"спасибо", "благодар", "признател", "отлично работа", "молодц",
		"довол", "рад ", "великолепн",
	}, 9},
	{"Жалоба", []string{
		"жалоба", "жалоб", "претензи", "недовол", "возмущ",
		"требую", "безобрази", "хамств", "нарушен", "обман",
	}, 8},
	{"Техническая проблема", []string{
		"не работает", "ошибка", "сбой", "зависа", "не открыва",
		"баг", "глюч", "не загруж", "не могу войти", "не отображ",
		"техническ", "приложени",
	}, 7},
	{"Заявка", []string{
		"заявка", "заявлен", "прошу", "хочу", "хотел бы",
		"необходимо", "нужно", "смена данных", "изменить",
		"открыть счет", "закрыть счет", "перевод", "оформи",
	}, 6},
	{"Консультация", []string{
...
func suggestActions(ticketType string, segment *string) []string {
	...
	case "Заявка":
		return []string{
			...
			"Обработать заявку в установленные сроки",
		}
	case "Консультация":
		return []string{
	...
	case "Благодарность":
		return []string{
			...
			"Передать благодарность ответственному сотруднику",
		}
	case "Спам":
